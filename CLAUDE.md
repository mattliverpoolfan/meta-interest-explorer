...
**【更正】上面「瑜伽練習 vs 瑜伽經典，語意上本質不同」這個結論被使用者當場指出沒有真正回答問題**——使用者追問的是：這三個標籤跟搜尋詞（童顏針）的關聯性照理應該差不多，AI 的語意判斷／**收到的提示詞**到底出了什麼狀況才會讓它們分屬不同級？並直接表明看不出重複測試 5 次對回答這個問題有什麼幫助。這個提醒是對的——上面的對照實驗只證明了「分級結果可重現、不是隨機亂跳」，從頭到尾沒有回答「為什麼」，而「語意上本質不同，是合理區分」這個說法也只是另一個沒有驗證過的理論，不能算數。

改用真數據查證（直接打 `delivery_estimate`，不經快取）後發現：**真正的機制根本不是語意層面的合理區分，而是 `classifyAndTierResults_` 的 prompt（`GeminiClient.gs`）從頭到尾沒有把候選詞自己的 Meta 真實受眾規模傳給 AI**，AI 純粹看標籤的「名字」腦補關聯度，對這個標籤在 Meta 上根不根本查得到受眾、能不能拿去投放完全沒有概念：

| 標籤 | 真實受眾（`delivery_estimate` 實測） | 與種子重疊率 |
|---|---|---|
| 訶陀瑜伽 | 1,300,000 | 9.76% |
| 瑜伽經 | **0**（查無可投放受眾，不是規模小） | 0% |
| 瑜珈（靈性） | 8,400,000（約佔全台約 21M 母數的 47.68%） | 47.68%（大分類天然高重疊，跟「瑜珈（靈性）」本身規模太大有關，不代表有意義的關聯訊號——跟 `pickSeed_` 當初避開大型母分類是同一個道理） |

也就是說：「瑜伽經」被判得比較低，不是因為 AI 正確辨識出「經典文本 vs 修行實踐」這種語意差異，而是這個標籤在 Meta 上根本是個查無受眾的「幽靈標籤」，但 AI 完全不知道這件事，照樣純憑文字語意生出一個看起來煞有介事的關聯等級。**修法（已落地，`UnifiedSearch.gs`）**：既然 AI 沒有能力判斷「這個標籤能不能真的投放」，就不要指望靠改 prompt 讓它學會——改用機制面直接擋：分類迴圈裡重用既有的 `estimatedAudienceSize_()`，把真實受眾 `<= 0` 的候選詞從直接/間接相關結果裡濾掉（門檻比第三類重疊掃描寬鬆很多，第三類要求統計上有意義的規模，這裡只擋真正查無受眾的，其餘只要有受眾就保留，因為第一二類的目的是給使用者真實可投放的選項）。

**對使用者原本問題的直接回答**：AI 的語意判斷本身沒有「壞掉」或不一致——它是穩定的、可重現的（這是重複測試唯一證明到的事）；問題出在**提示詞的資訊缺口**：AI 被要求對一個它只看得到「名字」、看不到「這個名字在 Meta 上到底代表多大/存不存在的受眾」的東西做關聯性判斷，這就是為什麼語意上看起來很相近的標籤可能被分到完全不同等級——因為分級的依據從頭到尾就不是真實的受眾輪廓重疊，只是文字聯想。重複測試 5 次的價值僅止於排除「這是隨機亂跳/模型不同」這兩個可能性，並不能回答「為什麼」——要回答「為什麼」，唯一可靠的路是像這次一樣直接查 Meta 的真實數據，而不是對 AI 的輸出做更多輪的理論推測。
