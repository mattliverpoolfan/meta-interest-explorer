...
**1. 「訶陀瑜伽」判高關聯度、「瑜伽經」判推測性關聯，邏輯不一致？** 追到根因：**這是比較兩次不同 run 的結果，不是同一批資料的分類矛盾**。Gemini 的候選詞聯想本來就沒有固定種子（同一個查詢每次呼叫可能吐出不同的候選搜尋詞），這次重跑吐出的間接候選詞是「瑜珈」（不是「瑜伽」），Meta 搜尋這個詞只回了一筆「瑜珈（靈性）」，分類成中關聯度——跟使用者截圖裡看到的「訶陀瑜伽」「瑜伽經」是完全不同一批候選詞、不同一次分類判斷。這是已知的 AI 機率性誤差（見上面「AI 分類的機率性誤差」），沒有進一步的程式碼修法，只能靠更多 few-shot 案例或換更強模型緩解。
